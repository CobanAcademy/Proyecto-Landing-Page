...
export default function Curso1Form() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = `https://wa.me/59177889320?text=${encodeURIComponent(
        `*Inscripción CURSO INTENSIVO VIRTUAL POR ZOOM*\n\nNombre: ${form.nombre}\nCI: ${form.ci}\nTeléfono: ${form.telefono}\nCorreo: ${form.correo}`
      )}`;
    ...
    <section id="inscripcion" className="py-20 md:py-28 px-4 bg-[#292150]">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-14">
          ...
          <p className="text-white/60 text-lg">Completá el formulario y te contactamos por WhatsApp para confirmar tu inscripción.</p>
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-linear-to-br from-[#2D2659] to-[#3A3270] border border-[#6C55D7]/50 rounded-2xl p-7 sticky top-24">
              <div className="text-center mb-6">
                <div className="inline-block bg-[#EB5523]/20 border border-[#EB5523]/40 text-[#EB5523] px-3 py-1 rounded-full text-xs font-bold mb-4 tracking-wide">
                  🔥 PRECIO ESPECIAL
                </div>
                <div className="text-6xl font-archivo-black text-white">350</div>
              ...
              <div className="space-y-3 mb-6">
                {[
                  ...
                  'Clase bonus: Clínica del Crédito',
                  'Clases 100% grabadas',
                  '2 asesorías personalizadas',
                  'Basado en normativa vigente',
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="w-5 h-5 bg-[#EB5523] rounded-full flex items-center justify-center shrink-0">
                      <CheckIcon />
                    </span>
              ...
              <div className="border-t border-white/10 pt-5 space-y-2 text-center">
                <p className="text-white/50 text-xs">📅 18, 20 y 22 de Mayo</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-bold text-lg transition-all ${
                    isValid && !sending
                      ? 'bg-[#EB5523] hover:bg-[#d44a1f] text-white shadow-lg hover:shadow-[#EB5523]/30 hover:-translate-y-0.5'
