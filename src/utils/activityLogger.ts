...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://api.cobanacademy.com";
...
export const useActivityLogger = () => {
  ...
  const logSectionNavigation = (
    section: string,
    source: "header" | "footer",
  ) => {
    activityLogs(
      ActivityCodes.SECTIONS,
      {
  ...
  const logAppStoreClick = (
    platform: "android" | "ios" | "web",
    source: "header" | "footer" | "dowloadApp",
  ) => {
    const code =
      platform === "android"
        ? ActivityCodes.ANDROID_DOWNLOAD
        : ActivityCodes.IOS_DOWNLOAD;
    activityLogs(
      code,
      {
